...
int main( int argc, char** argv )
{
    ...
    *******************************/
    ...
    v->setId( currIndex );
    v->setEstimate( Eigen::Isometry3d::Identity() ); //估计为单位矩阵 ，第一帧设置为世界坐标系，旋转为单位帧
    v->setFixed( true ); //第一个顶点固定，不用优化
    globalOptimizer.addVertex( v );
    
    keyframes.push_back( currFrame );//第一帧加入到关键帧中
    
    double keyframe_threshold = atof( pd.getData("keyframe_threshold").c_str() );
    bool check_loop_closure = pd.getData("check_loop_closure")==string("yes");//是否需要回环检测

    //上面就是系统的初始化的操作，设置了第一帧为世界坐标系。下面开始读取第二帧。。。。
    for ( currIndex=startIndex+1; currIndex<endIndex; currIndex++ )
    {
        cout<<"Reading files "<<currIndex<<endl;
        FRAME currFrame = readFrame( currIndex,pd ); // 读取currFrame
        computeKeyPointsAndDesp( currFrame, detector, descriptor ); //提取特征
        CHECK_RESULT result = checkKeyframes( keyframes.back(), currFrame, globalOptimizer ); //匹配该帧与keyframes里最后一帧       vector.back()返回最后一个元素
...
//这里是将所有图片都弄完了之后，也就是所有的边和顶点都弄好了之后，再做优化，整个系统也就这一个pose graph

    // 优化
    cout<<RESET"optimizing pose graph, vertices: "<<globalOptimizer.vertices().size()<<endl;
    globalOptimizer.save("./result_before.g2o");//将优化前的边和顶点 保存起来
    globalOptimizer.initializeOptimization();
    globalOptimizer.optimize( 100 ); //可以指定优化步数
    globalOptimizer.save( "./result_after.g2o" );//将优化后的在保存起来
...
double normofTransform( cv::Mat rvec, cv::Mat tvec )
{//fabs求小数的绝对值，abs求整数的绝对值   rvec 3x1
...
CHECK_RESULT checkKeyframes( FRAME& f1, FRAME& f2, g2o::SparseOptimizer& opti, bool is_loops)
{
    ...
    // 比较f1 和 f2
    RESULT_OF_PNP result = estimateMotion( f1, f2, camera );//计算这两帧之间的R t ，并保存在这个result结构体中
    ...
    // 向g2o中增加这个顶点与上一帧联系的边   pose graph  顶点为 两个相机的位姿，  边 是误差 为 两个顶点之间的相对运动 T
    // 顶点部分
    // 顶点只需设定id即可
    if (is_loops == false)
    {
