...
public:
    ...
    //! return the banner string。用于对_banner成员进行读取和设定
...
protected:
    ...
    /**
     * 这里说一下概况和设计思路。
     * _args[]为参数描述数组，往这个数组数组里面添加元素用param()，
     ...
     * _leftOvers[]参数数组需要的元素，也就是什么参数要事先用paramLeftOver()函数push进去(本程序貌似没用到)，
     * 要求在程序执行命令行中必须提供此类参数的值，不然报错！
     * _leftOversOptional为事先定义好的可选的参数数组，也是通过paramLeftOver()函数添加元素，
     * 与_leftOvers[]不同的是，它步强制命令行必须要提供参数值，命令行提供值与否不重要，也有可无。
