...
/// 位姿顶点
class VertexPose : public g2o::BaseVertex<6, SE3> { // 继承g2o::BaseVertex<6, SE3>
   public:
    //Eigen 数据对齐，固定写法
    ...
    //顶点更新函数。用于优化过程中增量x 的计算。我们根据增量方程计算出增量之后，通过这个函数对估计值进行调整的
    virtual void oplusImpl(const double *update) override {
        Vec6 update_eigen;
        update_eigen << update[0], update[1], update[2], update[3], update[4],
            update[5];
        _estimate = SE3::exp(update_eigen) * _estimate;
    }//更新方式： 更新小量 乘以 估计值。 左乘更新
    //读写操作。 按需自定义，一般不需要进行读写的话，仅仅声明一下就可以了
